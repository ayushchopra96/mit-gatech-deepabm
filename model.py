...
########################################################################################################
    ...
    def collect_results_trainable(self):
        plot_data = []
        for t in range(self.params['num_steps']):
            plot_data.append(make_one_hot(self.agents_stages[t, :], self.params['num_stages']).sum(axis=0))
        plot_data = torch.stack(plot_data,axis=0)
        total_infected = plot_data.sum(1) - plot_data[:,0] # 0 is S
        ...
        new_death = torch.diff(total_death,prepend=total_death[0:1])
        return {'new_infected':new_infected,'new_hosp':new_hosp,'new_death':new_death}
